...
class Game extends EventDispatcher {
    ...
    createScene() {
        ...
        World.player = new Mesh(
            new BoxGeometry(1, 1, 1),
            new MeshLambertMaterial({color: 0x0000ff, transparent: true, opacity: 0.5})
        )
        ...
        const intensity = 1;
        const light = new HemisphereLight(skyColor, 0xFFFFFF, intensity);
        this.scene.add(light);
        ...
        // create red grid
        let grid = new GridHelper(SIZE, divisions, RED, RED);
        grid.position.z = -SIZE / 2 + SIZE;
        groundBoundingBox.add(grid)

        // create blue grid
        grid = new GridHelper(SIZE, divisions, BLUE, BLUE);
    ...
    createGate(color, GATE_SIZE) {
        const geometry = new TorusGeometry(GATE_SIZE, GATE_SIZE / 3, 32, 16);
        // red gate
        let material = new LineBasicMaterial({color: color});
        let gate = new Line(geometry, material);
        return gate;
    }
